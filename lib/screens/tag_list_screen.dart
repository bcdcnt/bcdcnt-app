  ...
  Future<void> _fetch() async {
    try {
      final data = await ApiClient.query(
        r'query { tags(first: 200, page: 1, orderBy: [{column: "views", order: DESC}]) { data { id name slug views } paginatorInfo { total } } }',
      );
      final raw = data['tags'];
                      ...
                      return InkWell(
                        ...
                        child: Container(
                          ...
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            const Text('#', style: TextStyle(fontSize: 12, color: AppColors.accentLight, fontWeight: FontWeight.w700)),
                            const SizedBox(width: 2),
                            Text(t['name'] ?? '', style: body(const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.text))),
                          ]),
                        ),
                      );
                    }).toList(),
